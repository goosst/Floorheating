...
hours_of_data=1;
  ...
  try
    ...
    expected_entities = strsplit(entity_id, ',');
    for i = 1:numel(expected_entities)
        entity = strrep(expected_entities{i}, '.', '_');
        if ~isfield(entity_data, entity)
            entity_data = 'error';
            most_recent_states = 'error';
            hass_time = sprintf('Missing data for entity: %s', expected_entities{i});
            return;
        end
    end
